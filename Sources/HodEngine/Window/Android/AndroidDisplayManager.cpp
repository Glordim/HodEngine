...
namespace hod::inline window
{
	...
	bool AndroidDisplayManager::Initialize(android_app* androidApp)
	{
		...
		_mainWindow = CreateMainWindow(_androidApp->window);

		return true;
	}

	void AndroidDisplayManager::CommandHandleStatic(android_app* androidApp, int32_t command)
	{
		static_cast<AndroidDisplayManager*>(androidApp->userData)->CommandHandle(command);
	}

	void AndroidDisplayManager::CommandHandle(int32_t command)
	{
		switch (command)
		{
