...
namespace
{
	...
	struct Limit final
	{
		static constexpr int Range = sizeof(Underlying_) <= 2 ? static_cast<int>(std::numeric_limits<Underlying_>::max()) - static_cast<int>(std::numeric_limits<Underlying_>::min()) + 1 : std::numeric_limits<int>::max();
