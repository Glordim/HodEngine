...
namespace HOD
{
	namespace RENDERER
	{
		...
		bool Mesh::loadFbx(const char* path)
		{
			...
			fbxsdk::FbxNode* fbxMeshNode = FindMeshInFbxNode(fbxRootNode);
			...
			{
				Vertex_3P_3C_3N_2UV_3TA& v1 = _vertices[_indices[(i * 3) + 0]];
				Vertex_3P_3C_3N_2UV_3TA& v2 = _vertices[_indices[(i * 3) + 1]];
				Vertex_3P_3C_3N_2UV_3TA& v3 = _vertices[_indices[(i * 3) + 2]];
		...
		fbxsdk::FbxNode* Mesh::FindMeshInFbxNode(fbxsdk::FbxNode* node)
		{
			...
			{
				fbxsdk::FbxNode* returnedNode = FindMeshInFbxNode(node->GetChild(i));
		...
		bool Mesh::LoadFromLines(std::vector<Line_3P_3C>& lines)
		{
			...
			_vertices.resize(lineCount * 2);

			for (size_t i = 0; i < lineCount; ++i)
			{
				size_t vertexIndex = i * 2;

				_vertices[vertexIndex + 0] = lines[i].vertices[0];
				_vertices[vertexIndex + 1] = lines[i].vertices[1];
			}
		...
		bool Mesh::LoadFromTriangles(std::vector<Tri_3P_3C>& triangles)
		{
			...
			_vertices.resize(triCount * 3);

			for (size_t i = 0; i < triCount; ++i)
			{
				size_t vertexIndex = i * 3;

				_vertices[vertexIndex + 0] = triangles[i].vertices[0];
				_vertices[vertexIndex + 1] = triangles[i].vertices[1];
				_vertices[vertexIndex + 2] = triangles[i].vertices[2];
			}
		...
		bool Mesh::LoadSkybox()
		{
			...
			_vertices[i].pos[0] = -1.0f; _vertices[i].pos[1] = 1.0f;  _vertices[i].pos[2] = -1.0f; ++i;
			_vertices[i].pos[0] = -1.0f; _vertices[i].pos[1] = -1.0f;  _vertices[i].pos[2] = -1.0f; ++i;
			_vertices[i].pos[0] = 1.0f; _vertices[i].pos[1] = -1.0f;  _vertices[i].pos[2] = -1.0f; ++i;
			_vertices[i].pos[0] = 1.0f; _vertices[i].pos[1] = -1.0f;  _vertices[i].pos[2] = -1.0f; ++i;
			_vertices[i].pos[0] = 1.0f; _vertices[i].pos[1] = 1.0f;  _vertices[i].pos[2] = -1.0f; ++i;
		...
		void Mesh::CalculateBoundingBox()
		{
			...
			for (Vertex_3P_3C_3N_2UV_3TA& vertex : _vertices)
			{
