...
namespace HOD
{
    RendererVulkan::~RendererVulkan()
    {
        ...
        {
            if (vkDeviceWaitIdle(this->_device) != VK_SUCCESS)
                fprintf(stderr, "Vulkan: DeviceWaitIdel failed!\n");
        }

        if (this->_acquireNextImageFence != VK_NULL_HANDLE)
            vkDestroyFence(this->_device, this->_acquireNextImageFence, nullptr);

        if (this->_imageAvailableSemaphore != VK_NULL_HANDLE)
            vkDestroySemaphore(this->_device, this->_imageAvailableSemaphore, nullptr);

        if (this->_renderFinishedSemaphore != VK_NULL_HANDLE)
        ...
        if (this->_commandPool != VK_NULL_HANDLE)
    ...
    {
        size_t swapChainFramebufferCount = this->_swapChainFramebuffers.size();
        ...
        size_t swapChainImageViewCount = this->_swapChainImageViews.size();
    ...
    bool RendererVulkan::GetAvailableGpuDevices(std::vector<GpuDevice*>* availableDevices)
    {
        ...
        if (this->_availableGpu.empty() == true)
        {
            uint32_t physicalDeviceCount = 0;
            vkEnumeratePhysicalDevices(this->_instance, &physicalDeviceCount, nullptr);

            if (physicalDeviceCount == 0)
                return false;

            this->_availableGpu.resize(physicalDeviceCount);

            std::vector<VkPhysicalDevice> physicalDevices(physicalDeviceCount);
            vkEnumeratePhysicalDevices(this->_instance, &physicalDeviceCount, physicalDevices.data());
            ...
            for (size_t i = 0; i < physicalDeviceCount; ++i)
            {
                ...
                VkGpuDevice& device = this->_availableGpu[i];
                ...
                for (size_t j = 0; j < queueFamilyCount; ++j)
                {
                    ...
                    if (queueFamily.queueCount > 0 && queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT)
                    {
                        ...
                        if (vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, (uint32_t)j, this->_surface, &presentSupport) != VK_SUCCESS)
                        {
                ...
                if (device.compatible == true)
                {
                    ...
                    if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, this->_surface, &capabilities) != VK_SUCCESS)
                    {
                    ...
                    else
                    {
                        ...
                        if (vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, this->_surface, &formatCount, nullptr) != VK_SUCCESS)
                        {
                        ...
                        if (formatCount != 0)
                        {
                            ...
                            if (vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, this->_surface, &formatCount, formats.data()) != VK_SUCCESS)
                            {
                                fprintf(stderr, "Vulkan: Unable to get Surface formats !\n");
                                device.compatible = false;
                            }
                        ...
                        if (vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, this->_surface, &presentModeCount, nullptr) != VK_SUCCESS)
                        {
                        ...
                        if (presentModeCount != 0)
                        {
                            ...
                            if (vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, this->_surface, &presentModeCount, presentModes.data()) != VK_SUCCESS)
                            {
                                fprintf(stderr, "Vulkan: Unable to get Surface present modes !\n");
                                device.compatible = false;
                            }
        ...
        size_t avalaibleDeviceCount = this->_availableGpu.size();
        availableDevices->resize(avalaibleDeviceCount);

        for (size_t i = 0; i < avalaibleDeviceCount; ++i)
        {
            availableDevices->at(i) = (GpuDevice*)(&this->_availableGpu[i]);
        }
    ...
    {
        this->_selectedGpu = (VkGpuDevice*)physicalDevice;
        ...
        for (size_t i = 0; i < uniformBufferCount; ++i)
        {
            ...
            if (set == 0)
            {
                this->_viewLayout.ExtractBlockUbo(compVert, resource);
            }
            else if (set == 1)
            {
                this->_modelLayout.ExtractBlockUbo(compVert, resource);
            }
        }
        ...
        for (size_t i = 0; i < textureCount; ++i)
        {
            ...
            if (set == 0)
            {
                this->_viewLayout.ExtractBlockTexture(compVert, resource);
            }
            else if (set == 1)
            {
                this->_modelLayout.ExtractBlockTexture(compVert, resource);
            }
        }
        ...
        for (size_t i = 0; i < uniformBufferCount; ++i)
        {
            ...
            if (set == 0)
            {
                this->_viewLayout.ExtractBlockUbo(compFrag, resource);
            }
            else if (set == 1)
            {
                this->_modelLayout.ExtractBlockUbo(compFrag, resource);
            }
        }
        ...
        for (size_t i = 0; i < textureCount; ++i)
        {
            ...
            if (set == 0)
            {
                this->_viewLayout.ExtractBlockTexture(compFrag, resource);
            }
            else if (set == 1)
            {
                this->_modelLayout.ExtractBlockTexture(compFrag, resource);
            }
        }

        this->_viewLayout.BuildDescriptorSetLayout();
        this->_modelLayout.BuildDescriptorSetLayout();

        this->_unlitVertexColorMaterial = this->CreateMaterial(vertexShader, fragmentShader, Material::Topololy::TRIANGLE);
        this->_unlitVertexColorMaterialInstance = this->CreateMaterialInstance(this->_unlitVertexColorMaterial);

        this->_unlitVertexColorLineMaterial = this->CreateMaterial(vertexShader, fragmentShader, Material::Topololy::LINE);
        this->_unlitVertexColorLineMaterialInstance = this->CreateMaterialInstance(this->_unlitVertexColorLineMaterial);

        this->_skyboxMesh = new VkMesh();
        this->_skyboxMesh->LoadSkybox();
    ...
    bool RendererVulkan::CreateDevice()
    {
        ...
        queueCreateInfo.queueFamilyIndex = this->_selectedGpu->graphicsAndPresentQueueFamilyIndex;
        ...
        if (vkCreateDevice(this->_selectedGpu->physicalDevice, &createInfo, nullptr, &this->_device) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to create logical device !\n");
            return false;
        }

        vkGetDeviceQueue(this->_device, this->_selectedGpu->graphicsAndPresentQueueFamilyIndex, 0, &this->_graphicsQueue);
        vkGetDeviceQueue(this->_device, this->_selectedGpu->graphicsAndPresentQueueFamilyIndex, 0, &this->_presentQueue);
    ...
    {
        return this->_viewLayout.GetDescriptorSetLayout();
    }

    VkDescriptorSetLayout RendererVulkan::GetVkModelDescriptorSet() const
    {
        return this->_modelLayout.GetDescriptorSetLayout();
    }
    ...
    bool RendererVulkan::CreateSwapChain()
    {
        ...
        if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR(this->_selectedGpu->physicalDevice, this->_surface, &capabilities) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to get Surface capabilities !\n");
            return false;
        }
        ...
        if (this->_depthTexture.BuildDepth(extent.width, extent.height) == false)
            return false;
        ...
        this->_swapChainImageViews.resize(imageCount, VK_NULL_HANDLE);
        for (size_t i = 0; i < imageCount; ++i)
        {
            ...
            if (vkCreateImageView(this->_device, &imageCreateInfo, nullptr, &this->_swapChainImageViews[i]) != VK_SUCCESS)
            {
        ...
        for (size_t i = 0; i < imageCount; i++)
        {
            VkImageView attachments[] = {
                ...
                this->_depthTexture.GetTextureImageView()
            };
    ...
    bool RendererVulkan::CreateCommandPool()
    {
        ...
        poolInfo.queueFamilyIndex = this->_selectedGpu->graphicsAndPresentQueueFamilyIndex;
        ...
        if (vkCreateDescriptorPool(this->_device, &descriptorPoolInfo, nullptr, &this->_descriptorPool) != VK_SUCCESS)
        {
    ...
    bool RendererVulkan::CreateSemaphores()
    {
        ...
        if (vkCreateSemaphore(this->_device, &semaphoreInfo, nullptr, &this->_renderFinishedSemaphore) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to create semaphores!\n");
            return false;
        }
        ...
        if (vkCreateFence(this->_device, &fenceCreateInfo, nullptr, &this->_acquireNextImageFence) != VK_SUCCESS)
        {
    ...
    bool RendererVulkan::CreateBuffer(VkDeviceSize bufferSize, VkBufferUsageFlags bufferUsage, VkMemoryPropertyFlags memoryProperties, VkBuffer* buffer, VkDeviceMemory* bufferMemory)
    {
        ...
        if (vkBindBufferMemory(this->_device, *buffer, *bufferMemory, 0) != VK_SUCCESS)
        {
    ...
    bool RendererVulkan::CreateSampler(VkSampler* sampler)
    {
        *sampler = VK_NULL_HANDLE;
        ...
        if (vkCreateSampler(this->_device, &samplerInfo, nullptr, sampler) != VK_SUCCESS)
        {
    ...
    bool RendererVulkan::BeginSingleTimeCommands(VkCommandBuffer* commandBuffer)
    {
        *commandBuffer = VK_NULL_HANDLE;
        ...
        if (vkAllocateCommandBuffers(this->_device, &allocInfo, commandBuffer) != VK_SUCCESS)
            return false;
    ...
    {
        for (uint32_t i = 0; i < this->_selectedGpu->memProperties.memoryTypeCount; ++i)
        {
            if ((memoryTypeBits & (1 << i)) && (this->_selectedGpu->memProperties.memoryTypes[i].propertyFlags & memoryProperties) == memoryProperties)
            {
                *memoryTypeIndex = i;
                return true;
            }
    ...
    bool RendererVulkan::GenerateCommandBufferFromRenderQueue(RenderQueue& renderQueue, VkCommandBuffer* commandBuffer, std::vector<DescriptorSet*>& descriptorSetToCleanAfterRender)
    {
        ...
        renderPassInfo.framebuffer = this->_swapChainFramebuffers[this->_currentImageIndex];
        ...
        viewDescriptorSet->SetLayout(&this->_viewLayout);
        ...
        VkMesh* mesh = (VkMesh*)this->_skyboxMesh;
        ...
        VkMaterial* defaultMat = ((VkMaterialInstance*)this->_unlitVertexColorMaterialInstance)->GetMaterial();
        ...
        for (size_t i = 0; i < meshCount; ++i)
        {
            ...
            {
                materialInstance = (VkMaterialInstance*)this->_unlitVertexColorMaterialInstance;
            }
            ...
            modelDescriptorSet->SetLayout(&this->_modelLayout);
        ...
        for (size_t i = 0; i < lineCount; ++i)
        {
            ...
            if (materialInstance == nullptr)
            {
                materialInstance = (VkMaterialInstance*)this->_unlitVertexColorLineMaterialInstance;
            }
    ...
    bool RendererVulkan::SubmitRenderQueue(RenderQueue& renderQueue)
    {
        ...
        if (vkCreateFence(this->_device, &fenceCreateInfo, nullptr, &fence) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to create Fence!\n");
            return false;
        }
        ...
        if (vkQueueSubmit(this->_graphicsQueue, 1, &submitInfo, fence) != VK_SUCCESS)
        {
        ...
        if (vkWaitForFences(this->_device, 1, &fence, VK_TRUE, std::numeric_limits<uint64_t>::max()) != VK_SUCCESS)
        {
    ...
    {
        if (this->_swapChain == nullptr) // For exemple if the window is hidden the size will be 0 and not swap chain are created
            return false;

        if (vkResetFences(this->_device, 1, &this->_acquireNextImageFence) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to reset fence!\n");
            return false;
        }

        VkResult result = vkAcquireNextImageKHR(this->_device, this->_swapChain, std::numeric_limits<uint64_t>::max(), VK_NULL_HANDLE, this->_acquireNextImageFence, &this->_currentImageIndex);
        ...
        if (vkWaitForFences(this->_device, 1, &this->_acquireNextImageFence, VK_TRUE, std::numeric_limits<uint64_t>::max()) != VK_SUCCESS)
        {
            fprintf(stderr, "Vulkan: Unable to wait fence\n");
            return false;
        }
